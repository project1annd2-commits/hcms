...
export const login = async (username: string, password: string): Promise<{ user: User; permissions: Permission; error?: string } | null> => {
  try {
    ...
    if (apiUrl) {
      ...
      if (response.ok) {
        ...
        const userWithPermissions = { user: data.user as User, permissions: (data.permissions || {}) as Permission };
    ...
    // Server not available, fall back to Firestore
  }
  ...
  try {
    ...
    if ((!users || users.length === 0) && username === 'admin') {
      ...
      // Re-query
      users = await db.find<any>('users', { username: 'admin' }, { limit: 1 });
    }
 ...
 */
export const participantLogin = async (userId: string, type: string, password: string): Promise<{ success: boolean; error?: string } | null> => {
  try {
