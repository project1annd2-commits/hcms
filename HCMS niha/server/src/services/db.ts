...
export class DatabaseService {
    ...
    async find<T extends Document>(
        ...
        const processedFilter = this.convertObjectIds(filter);
    ...
    async findById<T extends Document>(
        ...
        // Handle both MongoDB ObjectIds and custom ID strings (e.g. from Firestore)
        let filter: any;
        if (isValidObjectId(id)) {
            filter = { _id: new ObjectId(id) };
        } else {
            // Check both _id (if saved as string) and the id field
            filter = { $or: [{ _id: id as any }, { id: id }] };
        }
    ...
    async updateOne<T extends Document>(
        ...
        const processedFilter = this.convertObjectIds(filter);
        const result = await collection.updateOne(processedFilter, updateDoc);
